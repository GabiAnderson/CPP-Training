/*
    ...
    Looking into the cmath library
        std::floor() - round down
        std::ceil() - round up
        std::abs() - get absolute value
        std::cos() - trig cos
        std::exp() - compute exponent
        std::log() - perform log
        std::pow() - raise number to power
        std::sqrt() - compute square root
        std::round() - round to nearest
        std::sin() - trig sin
        std::tan() - trig tan

    There are even more - https://en.cppreference.com/w/cpp/header/cmath
...
{
    double weight{7.7};
    std::cout << "Initial value: " << weight << std::endl;
    ...
    std::cout << "Abs of 7.7 is : " << std::abs(weight) << std::endl; // 7.7
    std::cout << "Abs of -5000 is : " << std::abs(savings) << std::endl; // 5000

    // exp : f(x) = e ^ x , where e = 2.71828 . Test the result here against a calculator
    double exponential = std::exp(10); // 22026.5 = 2.71828^10
